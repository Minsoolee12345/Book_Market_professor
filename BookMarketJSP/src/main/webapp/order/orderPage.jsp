...
<%
	CartService cartService = new MSCartService(new OracleCartDAO());
	BookService bookService = new MSBookService(new OracleBookDAO());
	MemberService memberService = new MSMemberService(new OracleMemberDAO());
...
<%@ include file = "/common/header.jsp" %>
	...
	<form action="order.jsp" method="post">
		...
		모바일 : <input type="text" value="<%= member.getMobile() == null ? "" : member.getMobile() %>"><br>
		이메일주소 : <input type="text" value="<%= member.getEmail() == null ? "" : member.getEmail() %>"><br>
		주소 : <input type="text" value="<%= member.getAddress() == null ? "" : member.getAddress() %>"><br>
